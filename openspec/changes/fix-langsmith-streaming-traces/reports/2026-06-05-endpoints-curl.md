...
## 5. Verificación de tracing en LangSmith (MANUAL — pendiente del usuario)
La corrección del árbol de nodos solo es verificable visualmente en la UI de LangSmith y
requiere tracing activo (`LANGCHAIN_TRACING_V2=true` + `LANGSMITH_API_KEY`). Tras lanzar una
petición a `/api/butler/ask-stream`, en el run `butler-text-stream` deben aparecer **anidados**
los nodos `classify_intent`, `speak_action`/`retrieve_context`/`answer_question` (según
intención), en lugar de una traza plana input→output. Esta comprobación queda a confirmación
visual del usuario; el agente no tiene acceso a la UI de LangSmith.
